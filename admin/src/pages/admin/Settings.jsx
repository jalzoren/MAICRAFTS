...
// ─────────────────────────────────────────────────────────────────────────────
// Hardcoded defaults — swap these out when connecting to a real backend later
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Toggle Row — reusable checkbox/switch row
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Number Stepper — reusable +/- control
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Custom Number Input with Stepper
// ─────────────────────────────────────────────────────────────────────────────
...
// ─────────────────────────────────────────────────────────────────────────────
// Special Character Set Selector
// ─────────────────────────────────────────────────────────────────────────────
...
const PasswordComplexitySection = () => {
  ...
  const getRequirements = () => [
    { 
      met: settings.minLength >= (settings.minLength || 8),    
      label: `At least ${settings.minLength} characters in length`,
      active: true
    },
    { 
      met: settings.requireUppercase,   
      label: settings.requireUppercase 
        ? `At least ${settings.uppercaseMinCount} CAPITAL letter${settings.uppercaseMinCount > 1 ? 's' : ''} (A-Z)`
        : "No uppercase letters required",
      active: settings.requireUppercase
    },
    { 
      met: settings.requireLowercase,   
      label: settings.requireLowercase 
        ? `At least ${settings.lowercaseMinCount} small letter${settings.lowercaseMinCount > 1 ? 's' : ''} (a-z)`
        : "No lowercase letters required",
      active: settings.requireLowercase
    },
    { 
      met: settings.requireNumber,      
      label: settings.requireNumber 
        ? `At least ${settings.numberMinCount} number${settings.numberMinCount > 1 ? 's' : ''} (0-9)`
        : "No numbers required",
      active: settings.requireNumber
    },
    { 
      met: settings.requireSpecialChar, 
      label: settings.requireSpecialChar 
        ? `At least ${settings.specialCharMinCount} special character${settings.specialCharMinCount > 1 ? 's' : ''}`
        : "No special characters required",
      active: settings.requireSpecialChar
    },
    {
      met: settings.expiresInDays > 0,
      label: settings.expiresInDays > 0
        ? `Password expires every ${settings.expiresInDays} days`
  ...
  const validateSettings = () => {
    if (settings.minLength < 8 || settings.minLength > 32) {
      Swal.fire({ 
        ...
        text: "Minimum length must be between 8 and 32.", 
        icon: "warning", 
    ...
    if (settings.requireUppercase && (settings.uppercaseMinCount < 1 || settings.uppercaseMinCount > 10)) {
      Swal.fire({ 
        title: "Invalid Value", 
        text: "Uppercase letters count must be between 1 and 10.", 
        icon: "warning", 
        confirmButtonColor: "#4b2e16" 
      });
      return false;
    ...
    if (settings.requireLowercase && (settings.lowercaseMinCount < 1 || settings.lowercaseMinCount > 10)) {
      Swal.fire({ 
        title: "Invalid Value", 
        text: "Lowercase letters count must be between 1 and 10.", 
        icon: "warning", 
        confirmButtonColor: "#4b2e16" 
      });
      return false;
    ...
    if (settings.requireNumber && (settings.numberMinCount < 1 || settings.numberMinCount > 10)) {
      Swal.fire({ 
        title: "Invalid Value", 
        text: "Numbers count must be between 1 and 10.", 
        icon: "warning", 
        confirmButtonColor: "#4b2e16" 
      });
      return false;
    ...
    if (settings.requireSpecialChar && (settings.specialCharMinCount < 1 || settings.specialCharMinCount > 10)) {
      Swal.fire({ 
        title: "Invalid Value", 
        text: "Special characters count must be between 1 and 10.", 
        icon: "warning", 
        confirmButtonColor: "#4b2e16" 
      });
      return false;
  ...
  const handleSave = async () => {
    ...
    setTimeout(() => {
      ...
      Swal.fire({ 
        title: "Saved!", 
        text: "Password complexity settings updated successfully.", 
        icon: "success", 
        confirmButtonColor: "#4b2e16",
        timer: 2000,
  ...
  const generateExamplePassword = () => {
    ...
    if (settings.requireUppercase) {
      password += "A".repeat(settings.uppercaseMinCount);
    }
    if (settings.requireLowercase) {
      password += "a".repeat(settings.lowercaseMinCount);
    }
    if (settings.requireNumber) {
      password += "1".repeat(settings.numberMinCount);
    }
    ...
    // Shuffle the password for better example
        ...
        <div className="ss-grid">
          ...
          <div className="ss-controls">
            <p className="ss-section-desc">
              Define the password rules enforced for all user accounts.
            </p>

            <NumberStepper
              label="Minimum Password Length"
              description="Recommended: 12 characters or more"
            ...
            <ToggleRow
              ...
              description="Require capital letters (A-Z)"
            ...
            {settings.requireUppercase && (
              <CustomNumberInput
                ...
                description="How many capital letters required"
                value={settings.uppercaseMinCount}
                onChange={(v) => set("uppercaseMinCount", v)}
                min={1} 
                max={10}
              />
            )}
            ...
            <ToggleRow
              ...
              description="Require small letters (a-z)"
            ...
            {settings.requireLowercase && (
              <CustomNumberInput
                ...
                description="How many small letters required"
                value={settings.lowercaseMinCount}
                onChange={(v) => set("lowercaseMinCount", v)}
                min={1} 
                max={10}
              />
            )}
            ...
            {settings.requireNumber && (
              <CustomNumberInput
                ...
                description="How many digits required"
                value={settings.numberMinCount}
                onChange={(v) => set("numberMinCount", v)}
                min={1} 
                max={10}
              />
            )}
            ...
            <ToggleRow
              ...
              onChange={(v) => set("requireSpecialChar", v)}
            />
            ...
            {settings.requireSpecialChar && (
              <>
                <CustomNumberInput
                  ...
                  description="How many symbols required"
                  value={settings.specialCharMinCount}
                  onChange={(v) => set("specialCharMinCount", v)}
                  min={1} 
                  max={10}
                />
                
                <SpecialCharSelector
                  value={settings.specialCharSet}
                  onChange={(v) => set("specialCharSet", v)}
                  disabled={!settings.requireSpecialChar}
                />
              </>
            )}
            ...
            <NumberStepper
              ...
              description="Days before users must reset their password. Set 0 to never expire."
              value={settings.expiresInDays}
              onChange={(v) => set("expiresInDays", v)}
              min={0} 
              max={365}
              unit={settings.expiresInDays === 0 ? "(never)" : "days"}
            />
          </div>
          ...
          <div className="ss-preview">
            ...
            <p className="ss-preview-subhead">
              Users will be required to create passwords that meet all enabled rules:
            </p>
            ...
            <div className={`ss-strength-badge ${strengthLabel.cls}`}>
              {strengthLabel.text}
            </div>
            
            {/* Example Password Display */}
            <div className="ss-example-password">
              <div className="ss-preview-header">Example Valid Password</div>
              <code className="ss-example-code">
                {generateExamplePassword()}
              </code>
              <p className="ss-example-note">
                * This is just an example. Actual passwords can be different.
              </p>
            </div>
          </div>
        </div>
...
const LoginAttemptsSection = () => {
  ...
  const handleSave = () => {
    if (settings.maxAttempts < 1 || settings.maxAttempts > 20) {
      Swal.fire({ 
        title: "Invalid Value", 
        text: "Max attempts must be between 1 and 20.", 
        icon: "warning", 
        confirmButtonColor: "#4b2e16" 
      });
      return;
    ...
    if (settings.lockoutDurationMinutes < 1 || settings.lockoutDurationMinutes > 1440) {
      Swal.fire({ 
        title: "Invalid Value", 
        text: "Lockout duration must be between 1 and 1440 minutes.", 
        icon: "warning", 
        confirmButtonColor: "#4b2e16" 
      });
      return;
    ...
    setTimeout(() => {
      ...
      Swal.fire({ 
        title: "Saved!", 
        text: "Login attempt settings updated successfully.", 
        icon: "success", 
        confirmButtonColor: "#4b2e16",
        timer: 2000,
        timerProgressBar: true
    ...
    <div className="ss-section">
      ...
      <div className="ss-section-body">
        <div className="ss-grid">
          ...
          <div className="ss-controls">
            <p className="ss-section-desc">
              Set how many failed login attempts are allowed before an account is temporarily locked.
            </p>

            <NumberStepper
              label="Maximum Failed Attempts"
              description="Account is locked after this many consecutive failures"
            ...
            <NumberStepper
              ...
              description="How long the account stays locked before the user can try again"
              value={settings.lockoutDurationMinutes}
              onChange={(v) => set("lockoutDurationMinutes", v)}
              min={1} 
              max={1440} 
              unit={settings.lockoutDurationMinutes >= 60 ? "hours" : "minutes"}
            />
          ...
          <div className="ss-preview">
            ...
            <div className="ss-attempt-summary">
              ...
              <div className="ss-attempt-stat">
                <span className="ss-attempt-num">
                  {settings.lockoutDurationMinutes >= 60
                    ? `${(settings.lockoutDurationMinutes / 60).toFixed(1)}h`
            ...
            {/* Attempt bubble timeline */}
            <div className="ss-attempt-timeline">
              {Array.from({ length: Math.min(settings.maxAttempts, 10) }).map((_, i) => (
                <div
                  key={i}
                  className={`ss-attempt-bubble ${
                    i === settings.maxAttempts - 1 ? "ss-attempt-bubble--lock"
                    : i >= settings.maxAttempts - 2 ? "ss-attempt-bubble--warn"
                    : "ss-attempt-bubble--ok"
                  }`}
                  title={`Attempt ${i + 1}`}
                >
                  {i + 1}
              ...
              {settings.maxAttempts > 10 && (
                <span className="ss-attempt-more">+{settings.maxAttempts - 10} more</span>
              )}
              <div className="ss-attempt-bubble ss-attempt-bubble--locked">🔒</div>
            </div>

            <div
              className="ss-strength-badge"
              style={{ 
